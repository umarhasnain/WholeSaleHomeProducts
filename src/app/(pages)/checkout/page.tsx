...
const BillingDetails: React.FC = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (formData.shipToDifferent) {
      ...
      const missingShippingField = shippingFields.find(
        (field) => !formData[field as keyof typeof formData]
      );
      if (missingShippingField) {
        toast.error("Please fill all required shipping fields.");
    ...
    try {
      ...
      const orderData = {
        ...
        createdAt: Timestamp.now(),
        shippingDetails: formData.shipToDifferent
          ? {
              shippingStreet: formData.shippingStreet,
              shippingCity: formData.shippingCity,
              shippingState: formData.shippingState,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="relative">
            ...
            <input
              ...
              onChange={handleChange}
              className="block w-full mt-1 p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
            />
          </div>
          <div className="relative">
            <label className="block text-sm font-medium text-gray-700">
